...
1. Create a D1 database named `todo-app-db`.
2. Copy its database ID into `wrangler.toml` as `database_id`.
3. In Cloudflare Dashboard, open **Workers & Pages → Create application → Worker → Import from Git** (or **Workers Builds → Create build**), connect `lilyyolanda/todo-app`, and select the `main` branch.
...
5. If the dashboard asks for a D1 binding, add one named `DB` and select `todo-app-db`. The binding name must be exactly `DB`.
